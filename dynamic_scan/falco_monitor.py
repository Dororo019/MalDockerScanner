...
Falco must be installed on the HOST (not inside the container): https://falco.org/docs/getting-started/installation/
Ubuntu: sudo apt install falco

PS: Falco works at the kernel syscall level using eBPF. It cannot run inside a container without privileged access, and it is not available in GitHub Actions runners.
"""
...
def run_falco_scan(
    ...
    """
    Check for Falco dynamic scanning:
